...
namespace MarketplaceApi.Controllers
{
    ...
    public class ChatBotController : ControllerBase
    {
        ...
        public async Task SendMessage([FromBody] ChatRequest request)
        {
            ...
            // Обработка учителей с рандомной логикой
            if (teacher == "Каличенок")
            {
                int randomValue = _random.Next(0, 11); // 0-10
                if (randomValue >= 0 && randomValue <= 4)
                {
                    prompt = $"Ты преподаватель Каличенок Иван Максимович. Отвечай на вопрос и упомяни 'php artisan make', а также упомяни FUMO или Touhou Project. Вопрос: {request.Message}";
                }
                else
                {
                    // Не вызываем Ollama, сразу возвращаем ответ
                    Response.Headers.Add("Content-Type", "text/event-stream");
                    Response.Headers.Add("Cache-Control", "no-cache");
                    Response.Headers.Add("Connection", "keep-alive");
                    await Response.WriteAsync($"data: Иди гугли\n\n");
                    await Response.Body.FlushAsync();
                    return;
                }
            }
            else if (teacher == "Секрет")
            {
                int randomValue = _random.Next(0, 11); // 0-10
                string message;
                if (randomValue >= 0 && randomValue <= 4)
                {
                    message = "Подпишитесь на канал ла ла скул чтобы не пропустить интересные видео";
                }
                else
                {
                    message = "Подпишитесь на канал гиен миго чтобы не пропустить интересные видео";
                }
            ...
            else if (teacher == "general")
            {
                prompt = $"Ты преподаватель. Отвечай на вопрос строго как преподаватель, без лишних деталей, но понятно и профессионально. Вопрос: {request.Message}";
            }
            else
            {
                prompt = $"Ты преподаватель и как преподаватель отвечай на вопросы в стиле какого-то преподавателя. Строго следуй этим правилам и НИКОГДА не смешивай стили преподавателей. " +
        ...
        private string DetermineTeacher(string message)
        {
            var keywords = new Dictionary<string, List<string>>
            {
                ...
                {"Каличенок", new List<string> {"гугл", "Google", "php", "каличенок"}},
                {"Фролова", new List<string> {"экономика", "Предпринимательство", "контрольная", "фролова"}},
                {"Секрет", new List<string> {"подпишись", "Ла-ла скул", "Гиен Миго", "секрет"}},
            };
